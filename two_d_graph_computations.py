...
def generate_random_coordiante(coordinates_dict: dict[str:tuple[int, int]]) -> tuple[int, int]:
    ...
    Preconditions:
        - all(0 <= coordinates_dict[key][0] <= constants.DISPLAY_WIDTH for key in coordinates_dict)
        - all(0 <= coordinates_dict[key][1] <= constants.DISPLAY_HEIGHT for key in coordinates_dict)
    """
    random_tuple = (
        round(random.uniform(constants.RADIUS, constants.DISPLAY_WIDTH - constants.RADIUS)),
        round(random.uniform(constants.RADIUS, constants.DISPLAY_HEIGHT - constants.RADIUS)))

    while any(math.dist(random_tuple, coordinates_dict[node]) < 3 * constants.RADIUS for node in coordinates_dict):
        random_tuple = (round(random.uniform(constants.RADIUS, constants.DISPLAY_WIDTH - constants.RADIUS)),
                        round(random.uniform(constants.RADIUS, constants.DISPLAY_HEIGHT - constants.RADIUS)))
